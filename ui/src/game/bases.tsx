...
import {TickCrew} from 'blitz2021/dist/game/types';
...
const Bases: React.FunctionComponent<{crews?: TickCrew[]}> = ({crews}) => {
    ...
    const bases = crews?.map((crew: TickCrew, i) => {
        const {x, y} = crew.homeBase;
        return (
            <Group key={`base-${i}`} x={x * Size.Tile} y={y * Size.Tile} ref={ref}>
                <BaseSafeZone color={colors[i]} safeZoneRadius={crew.safeZoneRadius} />
                <Rect
                    fill="#efe4d0"
                    width={Size.Tile}
                    height={Size.Tile}
